...
{
	//Create the window
	sf::RenderWindow window(sf::VideoMode(1200, 675), "CMP303 - Player 1");
	...
	unsigned short thisBulletsPort;
	...
	std::cout << "This player's application must be run first to allow the other player to connect \n";
	std::cout << "Please enter this player's IP and port. Must match the other's player's IP and port, IP and Port used for testing was 127.0.0.2 and 4444 \n";
	...
	std::cin >> thisBulletAddress >> thisBulletsPort;
	...
	level.network.initSockets(thisPlayerAddress, thisPlayersPort, thisBulletAddress, thisBulletsPort, thisStartIP, thisStartPort);
	level.network.sendStart();
	level.state = 0;
